...
import { Task, Runbook, ITCategory, IT_CATEGORIES, TaskStatus, UserRole, StaffMember } from '../types';
...
interface TaskModalProps {
  ...
  userRole?: UserRole;
  /** IT accounts a ticket can be assigned to. */
  staff?: StaffMember[];
}
...
export const TaskModal: React.FC<TaskModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // updatedAt stays as it was when the ticket was opened: the server uses it
    // to spot a change someone else saved in the meantime, then sets the new time.
    onSave(formData);
    onClose();
  };
              ...
              <div>
                ...
                {isReadOnly ? (
                  <input
                    type="text"
                    disabled
                    value={formData.assigneeId ? formData.assignee?.name : 'Unassigned'}
                  ...
                  <div className="relative">
                    {/* IT accounts only, so every assignment points at a real person. */}
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setFormData({
                          ...
                          assignee: member
                            ? { name: member.name, role: 'IT Technician', email: member.email }
                            : { name: 'Unassigned', role: '', email: '' },
                        });
                      }}
                      className="w-full appearance-none bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl px-3.5 py-2.5 text-xs font-semibold text-slate-800 dark:text-slate-200 focus:outline-none focus:border-indigo-500 pr-9"
                    >
